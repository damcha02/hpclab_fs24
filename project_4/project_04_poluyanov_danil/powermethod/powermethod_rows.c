...
int main(int argc, char* argv[]) {
  ...
  // 
  int rows_per_proc = n /size; // rows per process
  int remainder = n % size;
  if(rank < remainder){
    nrows_local = rows_per_proc + 1;
    row_beg_local = rank * nrows_local; 
  }
  else{
    nrows_local = rows_per_proc;
    row_beg_local = rank * nrows_local + remainder;
  }
  ...
  for (iter = 0; iter < niter; ++iter) {
    ...
    int* receivecount_ = malloc(size * sizeof(int));
// added this 
    int sum_ = 0;
    for(int i = 0; i < size; ++i){
      receivecount_[i] = (i < remainder) ? rows_per_proc + 1 : rows_per_proc;
      buff[i] = sum_;
      sum_ += receivecount_[i];
    }

    // synchronizing results
    // MPI_Allgather(sendbuf,sendcount,mpi_datatype, recvbuff, recvcount, mpi_datatype,mpi_comm comm);
    MPI_Allgatherv(y_local, nrows_local, MPI_DOUBLE, y, receivecount_, buff, MPI_DOUBLE, MPI_COMM_WORLD);
