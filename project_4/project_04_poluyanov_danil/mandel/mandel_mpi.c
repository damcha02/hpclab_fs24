...
int main(int argc, char** argv) {
	...
	{	
		...
		MPI_Request request;
		MPI_Isend(c, d.nx * d.ny, MPI_INT, 0, 0, MPI_COMM_WORLD, &request);
	...
	{	
		...
		MPI_Request *request = malloc((mpi_size - 1) * sizeof(MPI_Request));
		// buffer, also needs memory allocated 
		// array of pointers to integers
		// receive buffer is a pointer to a pointer of type int
		int** receive_buffer_ = malloc((mpi_size - 1) * sizeof(int*));
		...
		for (int proc = 1; proc < mpi_size; proc++)
		{
			...
			receive_buffer_[proc - 1] = malloc(d1.nx * d1.ny * sizeof(int));
			MPI_Irecv(receive_buffer_[proc - 1], d1.nx * d1.ny, MPI_INT, proc, 0, MPI_COMM_WORLD, &request[proc - 1]);
		}
		...
		for (int proc = 1; proc < mpi_size; proc++){
			...
			// MPI_Recv(&receive_buffer_, d1.nx * d1.ny, MPI_INT, proc, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			// write the partition of the process proc
			// MPI_Recv(receive_buffer_, 1, MPI_INT, proc, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
			Partition p1 = updatePartition(p, proc);
			Domain d1 = createDomain(p1);
			...
			for (j = 0; j < d1.ny; j++) // HEIGHT
			{
				...
				{
					int c_ij = receive_buffer_[proc - 1][index(i,j,d1.nx)];
					png_plot (pPng, i+d1.startx, j+d1.starty, c_ij ,c_ij, c_ij);
				}
			}
			free(receive_buffer_[proc - 1]);
		}
